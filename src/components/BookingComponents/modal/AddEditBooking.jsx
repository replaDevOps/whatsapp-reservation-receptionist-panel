...
import {
  GET_SERVICE_PROVIDER_BY_BRANCH,
  GET_SERVICES_BY_BRANCH_LOOKUP,
} from "../../../graphql/query/lookupsquery";
import { getBranchId } from "../../../utils/auth";
import {
  GET_APPOINTMENTS_BY_SERVICE_PROVIDER,
...
const AddEditBooking = ({ visible, onClose, edititem, refetch }) => {
  ...
  const [getServicesByBranchLookup, { data: servicesByBranchLookup }] =
    useLazyQuery(GET_SERVICES_BY_BRANCH_LOOKUP);
  const [
    getServiceProvidersByBranchLookup,
    { data: serviceProvidersByBranchLookup },
  ] = useLazyQuery(GET_SERVICE_PROVIDER_BY_BRANCH);
  const [
    getAppointmentsByServiceProvider,
    { data: appointmentsByServiceProvider },
  ] = useLazyQuery(GET_APPOINTMENTS_BY_SERVICE_PROVIDER);
  const [
    getVerifyPromotion,
    { data: verifyPromotionData, loading: verifyingPromotion },
  ] = useLazyQuery(VERIFY_PROMOTION_CODE);
  ...
  useEffect(() => {
    ...
    console.log(
      "appointments by service provider",
      appointmentsByServiceProvider?.getAppointmentsByServiceProvider,
    );
  ...
  function getAvailableTimeSlots(
    ...
    while (slotStart + slotDuration <= workEnd) {
      ...
      const overlapBlock = blocks.find(
        (b) => slotStart < b.end && slotComplete > b.start,
      );
  ...
  useEffect(() => {
    if (visible && edititem) {
      ...
      form.setFieldsValue({
        ...
        appointmentTime: edititem?.appointmentTime
          ? dayjs(edititem?.appointmentTime, "HH:mm")
          : null,
  ...
  const CreateBookingHandle = async () => {
    ...
    const payload = {
      ...
      appointmentTimeSlot:
        isAccess === "BY_SERVICE_PROVIDER" ? timeslotes : appointmentTime,
            ...
            <Row>
                    ...
                    validator(_, value) {
                      ...
                      if (phoneLength < 9 || phoneLength > 20) {
                        return Promise.reject(
                          new Error(
                            t("Phone number must be between 9 and 20 digits"),
                          ),
                        );
                      }
                      return Promise.resolve();
                  ...
                  addonBefore={
                    ...
                    >
                      ...
                      <Select.Option value="ae">
                        +{isArabic ? toArabicDigits(955) : 955}
                      </Select.Option>
                    </Select>
                  }
                ...
                <MyInput
                  ...
                  suffix={
                    <Flex align="center" gap={2}>
                      ...
                      <Tag onClick={checkPromoCode} className="cursor">
                        {t("Check")}
                      </Tag>
                    </Flex>
                  }
              ...
              <Col span={24}>
                <MySelect
                  ...
                  options={serviceProvidersByBranchLookup?.getServiceProvidersByBranch?.users?.map(
                ...
                <MyDatepicker
                  ...
                  onChange={(date) => {
                    const serviceProviderId =
                      form.getFieldValue("serviceProviderId");
                    if (serviceProviderId)
                      getAppointmentsByServiceProvider({
                        variables: { serviceProviderId, date },
                      });
              ...
              {isAccess === "BY_SERVICE_PROVIDER" ? (
                <Col span={24}>
                  <Flex vertical gap={5} className="my-3">
                    ...
                    <Flex
                      className="offday pad bg-light-gray"
                      gap={5}
                      align="center"
                      justify="space-between"
                    >
                      <Text className="text-gray fs-13">
                        {timeslotes ? timeslotes : t("Select time")}
                      </Text>
                      <img
