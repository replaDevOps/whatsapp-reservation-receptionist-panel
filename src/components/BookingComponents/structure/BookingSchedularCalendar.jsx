...
const BookingSchedularCalendar = () => {
  ...
  const [getAppointments, { data: appointData, loading: appointLoading }] =
    useLazyQuery(GET_BOOKINGS, { fetchPolicy: "network-only" });
  const [getServiceLookups, { data: lookupService }] = useLazyQuery(
    GET_SERVICE_BY_SERVICE_PROVIDER,
  );
  ...
  }, [getAppointments, selectedProvider, selectedService]);
  ...
  useEffect(() => {
    if (appointData?.getAppointments) {
      ...
      const result = appointData?.getAppointments?.appointments?.map(
        (item) => ({
  ...
  const normalizedEvents = appointments
    .map((ev) => {
      ...
      const endDate = new Date(
        dayjs(startDate)
          .add(ev?.service?.duration, "minutes")
        ...
        <Flex vertical gap={20}>
          ...
          <div className="position-relative">
            ...
            <Calendar
              ...
              resources={
                selectedProvider
                  ? serviceProviders.filter((sp) => sp.id === selectedProvider)
                  : serviceProviders
              }
