...
const StaffEventCard = ({ event }) => {
    ...
    const statusColors = {
        APPROVED: { bg: "#C4E6C8",color:"#008F17" },
        REJECTED: { bg: "#FFD2CF", color:"#FF3B30" },
        PENDING: { bg: "#F7DDC4", color:"#DB6E00" },
    };
    const startDate = moment(event.start).format("YYYY/MM/D");
    const endDate = moment(event.end).format("YYYY/MM/D");
    ...
    return(
        <Flex gap={20} vertical justify="end" className="h-100" align="end">
            ...
            >
                ...
                {isMultiDay ? (
                <Text className="fs-12 fw-500 text-white">
                    {startDate} - {endDate}
                </Text>
                ) : (
                <Text className="fs-12 fw-500 text-white">{startDate}</Text>
                )}
            </Flex>
        </Flex>
    );
