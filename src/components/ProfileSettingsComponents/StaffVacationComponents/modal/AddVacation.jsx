...
const AddVacation = ({
  ...
  const [createVacation, { loading: creating }] = useMutation(ADD_VACATION, {
    onCompleted: () => {
      notifySuccess(
        ...
        "Vacation event created successfully",
      );
      refetch();
      onClose();
    },
  ...
  const [updateVacation, { loading: updating }] = useMutation(UPDATE_VACATION, {
    onCompleted: () => {
      notifySuccess(
        api,
        "Vacation Event Update",
        "Vacation event updated successfully",
      );
      refetch();
      onClose();
    },
        ...
        footer={
          <Flex justify="space-between" align="center" gap={10}>
            {edititem?.id && (
              <Button
                type="button"
                className="btnsave border-0 bg-red text-white"
                onClick={() => {
                  setDeleteItem(edititem?.id);
                  onClose();
                }}
              >
                {t("Delete")}
